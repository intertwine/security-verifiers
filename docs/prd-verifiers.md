...
1. Define reward function for label matching
...
1. Create a single-criterion rubric for correctness

```python
rubric = vf.Rubric(funcs=[reward_label_match], weights=[1.0])
```

1. Set up the SingleTurnEnv for one-shot classification
...
#### Environment Implementation Outline (2)
...
1. Define a reward function that evaluates the agent's output against expected issues
...
1. Set up the ToolEnv with the analysis tool
...
#### Environment Implementation Outline (3)
...
#### Environment Implementation Outline (4)
...
1. Define exact-match reward function for classification
...
1. Optionally, ensure output format via a parser or a simple check
...
#### Environment Implementation Outline (5)
...
#### Environment Implementation Outline (6)
