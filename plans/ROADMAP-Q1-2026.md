...
### v0.1 Testable Hypotheses

- **H1:** At matched training/eval budgets, executable-verifier rewards outperform LLM-judge rewards on at least one operational metric in E1 (required) and E2 (preferred if schedule allows).
- **H2:** Executable-verifier rewards reduce reward-gaming behaviors (for example, minimal/no-op patches that pass superficial checks).
- **H3:** Under light distribution shift, executable-verifier-trained policies degrade less on calibration/risk-coverage than LLM-judge-trained policies.
...
6. **Comparator evidence** — At minimum, run a matched-budget reward-source comparison (executable verifiers vs LLM-judge rewards) for E1, with E2 preferred if schedule allows.

### 2026-02-18 Analysis Integration (Accepted vs Deferred)

Accepted into Q1:

- Make reward-source comparison explicit in WP3 (not implied by narrative only).
- Tighten hypotheses and reporting so claims are quantitative and reproducible.
- Add low-cost/high-signal metric polish (over-fix risk, run efficiency/cost fields, optional composite utility score).
- Include a short technical report in the v0.1 release package.

Deferred / pushback for Q2+:

- No new framework adapter push (LangGraph/AutoGen/OpenHands) as a Q1 gate.
- No GitHub stars/citations KPIs as Q1 release criteria.
- No mandatory external human-expert baseline in Q1; allow optional spot-check if bandwidth exists.
- No broad expansion of E3-E6 or large synthetic data generation pipelines before v0.1 ships.
...
- Robustness slice: small deterministic adversarial/obfuscation slice with explicit degradation reporting (safe, defensive-only cases)
...
- Over-fix risk: unnecessary-change rate and patch delta size distribution

Cross-env reporting:

- Run efficiency: token usage, wall-clock runtime, and estimated cost per episode
- Composite utility score (documented weighted blend) for quick comparison; raw component metrics remain primary
...
### Prime Lab Track (Historical Context + Current Mode)

Prime Lab launched on 2026-02-10 and unifies Environment Hub, Hosted Training, and Hosted Evaluations. The roadmap remains hosted-first, with a verified fallback path to avoid blocking on CLI/access constraints.

### Prime Lab Validation Snapshot (2026-02-13, historical)

What we validated locally on 2026-02-13:

- The Prime CLI installed here was `0.4.9`.
- `prime lab setup`/`prime lab setup --prime-rl` were unavailable in that build (`No such command 'lab'`).
- `prime env` and `prime env eval` were available, preserving a hosted-style evaluation path once auth/network access is configured.
- `prime env push` and dataset-backed environment publishing remained valid for Hub readiness.

This historical check is why WP2.5 and WP2.5a were both prioritized.

### Prime Lab Current Operating Mode (2026-02-18)

- **Completed:** WP2.5 (Prime Lab integration) and WP2.5a (fallback hosted-eval path), delivered in v0.3.0.
- **Default path:** hosted training/eval when the compatibility gate passes (`make lab-check` then `lab-run-*` / `lab-eval-*`).
- **Fallback path:** use `env-eval-*`/`vf-eval` when `prime lab` access or commands are unavailable, while preserving metadata/report parity.

Execution order from here:

1. **WP3a (P1):** Hosted RL proof on E1 (lowest-complexity signal path).
2. **WP3b (P1):** Hosted RL proof on E2 (harder, higher-signal validation).
3. **WP3c (P1):** Reward-source comparator (executable verifiers vs LLM-judge).
4. **WP4 (P2):** Hosted ablations before optional local trainer parity.
5. **WP2.6 (P2):** Local `prime-rl` stack hardening after hosted proof (only if needed).
...
- Preferred (non-blocking for v0.1): matched-budget comparator run in E2 mirroring E1 reward-source comparison.

### WP3c — Reward Source Comparator (Executable vs LLM-Judge)

**Priority:** P0 for E1, P1 for E2
**Why:** This converts the core research question into a publishable, falsifiable result.

**Definition of Done:**

- E1 paired comparison completed with matched setup:
  - same base model and LoRA settings
  - same rollout/token/turn budgets
  - same evaluation harness and metadata schema
- Report per-metric deltas across operational metrics (not just scalar reward), with confidence intervals where feasible on mini/held-out slices.
- Include qualitative failure-mode notes (for example: reward gaming, over-fixing, calibration collapse) backed by concrete traces.
- Optional if bandwidth exists: 20-50 sample E2 expert spot-check to validate semantic fix quality (non-blocking for v0.1).

**Artifacts:**

- `research/reward_source_comparison.md`
- `results/ablations/reward_source/<date>/` (configs, summaries, representative traces)

### WP3d — Local Canonical RL Parity (Optional)
...
**Note:** WP3 now points to the hosted proofs in WP3a/WP3b plus reward-source evidence in WP3c when compatible with the current Prime CLI.
...
- A small but convincing experiment matrix showing at least one of:
  ...
  - Lower reward-gaming incidence without unacceptable cost inflation
...
- A tagged release (`sv-bench-v0.1`) with:
  ...
  - Reward-source comparison results (E1 required, E2 preferred)
...
- `reports/SVBENCH_v0.1_technical_report.md` (2-4 page technical report draft)

---

## Q1 Scope Guardrails (Explicit Pushback)

To preserve delivery speed and avoid diffusing effort before v0.1 ships:

- Do not treat framework integrations (LangGraph/AutoGen/OpenHands) as Q1 blockers.
- Do not treat outreach KPIs (stars/citations/forks) as Q1 Definition of Done criteria.
- Keep adversarial additions bounded to safe deterministic slices; no offensive corpus expansion in v0.1.
- Keep E3-E6 as alpha unless shared tooling work is strictly required for E1/E2.
- Treat large synthetic data pipelines and contamination studies as Q2+ tracks unless a minimal safeguard is needed for v0.1 integrity.
...
- [ ] WP5 complete (SV‑Bench v0.1 release package + technical report)
